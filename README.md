...
| [DATA_SCHEMA.md](DATA_SCHEMA.md) | Data schemas for raw project inputs, engineered features, prediction outputs, and candidate data sourcing strategies. | Active |
...
- **Scientific Integrity:** No fabricated accuracy claims, invented statistics, or fake ministry endorsements. All training data strategies and target formulations remain pending data research and validation.
- **Architectural Pragmatism:** SQLite is adopted as the primary persistence engine for local demo simplicity, with PostgreSQL preserved as an optional future production path.
...
- **Persistence:** SQLAlchemy with **SQLite** (primary local/demo persistence; PostgreSQL as optional future deployment path)
- **Deployment:** Standalone local runtime / Docker Compose (optional future containerization path)
...
└── docker/                 # Containerization configs (optional future packaging)
```
...
- [ ] **Phase 1:** Data Research & Strategy Evaluation (Public extraction vs calibrated benchmark vs hybrid)
...
- [ ] **Phase 5:** Interactive React + TypeScript Dashboard (Executive Summary, Project Explorer, What-If Simulator [P1])
- [ ] **Phase 6:** End-to-End Testing and Demo Script Finalization
